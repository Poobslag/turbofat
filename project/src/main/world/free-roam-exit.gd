...
func _physics_process(_delta: float) -> void:
	if Engine.editor_hint or not ChattableManager.player:
	...
	if _player_overlapping and not SceneTransition.fading:
		...
		if player.non_iso_walk_direction and target_direction \
			...
			PlayerData.free_roam.player_spawn_id = player_spawn_id
			PlayerData.free_roam.sensei_spawn_id = sensei_spawn_id
			player.fade_out()
			
			PlayerData.free_roam.environment_path = destination_environment_path
