...
var _ghost_shadow_offset := Vector2i.ZERO
...
func _refresh_ghost_piece() -> void:
	if SystemData.gameplay_settings.ghost_piece:
		...
		_tile_map.set_ghost_shadow_offset(Vector2i.ZERO)


func _on_Dropper_hard_drop_target_pos_changed(piece: ActivePiece, hard_drop_target_pos: Vector2i) -> void:
