...
func is_cake_cell(cell: Vector2i) -> bool:
	return get_cell_source_id(0, cell) == TILE_BOX and Foods.is_cake_box(get_cell_atlas_coords(0, cell).y)


func is_snack_cell(cell: Vector2i) -> bool:
	return get_cell_source_id(0, cell) == TILE_BOX and Foods.is_snack_box(get_cell_atlas_coords(0, cell).y)


func set_ghost_shadow_offset(new_ghost_shadow_offset: Vector2i) -> void:
...
func save_state() -> void:
	...
	for cell_obj in get_used_cells(0):
		var cell: Vector2i = cell_obj
		_saved_used_cells.append(cell)
		_saved_tiles.append(get_cell_source_id(0, cell))
		_saved_autotile_coords.append(get_cell_atlas_coords(0, cell))
...
func restore_state() -> void:
	...
	for i in range(_saved_used_cells.size()):
		var cell: Vector2i = _saved_used_cells[i]
		var tile: int = _saved_tiles[i]
		var autotile_coord: Vector2i = _saved_autotile_coords[i]
...
func set_block(pos: Vector2i, tile: int, autotile_coord: Vector2i = Vector2i.ZERO) -> void:
	set_cell(0, pos, tile, autotile_coord)
	if is_inside_tree():
		corner_map.dirty = true
...
func build_box(rect: Rect2i, box_type: Foods.BoxType) -> void:
	for curr_x in range(rect.position.x, rect.end.x):
		for curr_y in range (rect.position.y, rect.end.y):
			var autotile_coord := Vector2i(15, box_type)
			...
			set_block(Vector2i(curr_x, curr_y), TILE_BOX, autotile_coord)


## Deletes the row at the specified location, lowering all higher rows to fill the gap.
func delete_row(y: int) -> void:
	erase_row(y)
	shift_rows(y - 1, Vector2i.DOWN)


## Erases the block from the specified cell, disconnecting any neighboring cells.
func erase_puzzle_cell(pos: Vector2i) -> void:
	if get_cell_source_id(0, pos) == -1:
		return

	set_block(pos, -1)
	_disconnect_from_empty_cells(pos + Vector2i.UP)
	_disconnect_from_empty_cells(pos + Vector2i.DOWN)
	_disconnect_from_empty_cells(pos + Vector2i.LEFT)
	_disconnect_from_empty_cells(pos + Vector2i.RIGHT)


## Inserts a blank row at the specified location, raising all higher rows to make room.
func insert_row(y: int) -> void:
	shift_rows(y, Vector2i.UP)
...
func is_cell_empty(pos: Vector2i) -> bool:
	return Rect2i(0, 0, COL_COUNT, ROW_COUNT).has_point(pos) and get_cell_source_id(0, pos) == -1


## Returns 'true' if the specified cell is outside the playfield bounds or contains a block.
func is_cell_obstructed(pos: Vector2i) -> bool:
...
func somewhere_near_cell(cell_pos: Vector2i, cell_offset: Vector2 = Vector2.ZERO) -> Vector2:
	if cell_offset == Vector2.ZERO:
		cell_offset = Vector2(randf(), randf())
	return (Vector2(cell_pos) + cell_offset) * Vector2(tile_set.tile_size) * scale
...
##
## 	'direction': The direction to shift the rows, such as Vector2i.UP or Vector2i.DOWN.
func shift_rows(bottom_row: int, direction: Vector2i) -> void:
	...
	for cell in get_used_cells(0):
		...
		var piece_color: int = get_cell_source_id(0, cell)
...
func contents_as_string() -> String:
	...
	for y in range(get_used_rect().position.y, get_used_rect().position.y + get_used_rect().size.y):
		...
		for x in range(get_used_rect().position.x, get_used_rect().position.x + get_used_rect().size.x):
			var cell_str: String = "."
			match get_cell_source_id(0, Vector2i(x, y)):
				-1: cell_str = "."
...
func crumb_colors_for_cell(cell_pos: Vector2i) -> Array:
	var result := []
	var autotile_coord := get_cell_atlas_coords(0, cell_pos)
	
	var cellv := get_cell_source_id(0, cell_pos)
	match cellv:
...
##
## Disconnected boxes have their connections updated. Disconnected pieces are converted to vegetable blocks.
func _disconnect_from_empty_cells(pos: Vector2i) -> void:
	match get_cell_source_id(0, pos):
		TILE_PIECE:
...
func _convert_piece_if_broken(pos: Vector2i) -> void:
	# store connections
	var old_autotile_coord: Vector2i = get_cell_atlas_coords(0, pos)
	
	var should_convert := false
	if PuzzleConnect.is_u(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.UP) != TILE_PIECE:
		should_convert = true
	if PuzzleConnect.is_d(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.DOWN) != TILE_PIECE:
		should_convert = true
	if PuzzleConnect.is_l(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.LEFT) != TILE_PIECE:
		should_convert = true
	if PuzzleConnect.is_r(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.RIGHT) != TILE_PIECE:
		should_convert = true
	
	if should_convert:
		# convert to vegetable. there are four kinds of vegetables
		var vegetable_type := int(old_autotile_coord.y) % 4
		set_block(pos, TILE_VEG, Vector2i(randi() % 18, vegetable_type))
		
		# recurse to neighboring connected cells
		if PuzzleConnect.is_u(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.UP) == TILE_PIECE:
			_convert_piece_if_broken(pos + Vector2i.UP)
		if PuzzleConnect.is_d(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.DOWN) == TILE_PIECE:
			_convert_piece_if_broken(pos + Vector2i.DOWN)
		if PuzzleConnect.is_l(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.LEFT) == TILE_PIECE:
			_convert_piece_if_broken(pos + Vector2i.LEFT)
		if PuzzleConnect.is_r(old_autotile_coord.x) and get_cell_source_id(0, pos + Vector2i.RIGHT) == TILE_PIECE:
			_convert_piece_if_broken(pos + Vector2i.RIGHT)
...
func _disconnect_box_from_empty_neighbors(pos: Vector2i) -> void:
	if get_cell_source_id(0, pos + Vector2i.UP) == -1:
		_disconnect_block(pos, PuzzleConnect.UP)
	if get_cell_source_id(0, pos + Vector2i.DOWN) == -1:
		_disconnect_block(pos, PuzzleConnect.DOWN)
	if get_cell_source_id(0, pos + Vector2i.LEFT) == -1:
		_disconnect_block(pos, PuzzleConnect.LEFT)
	if get_cell_source_id(0, pos + Vector2i.RIGHT) == -1:
...
## 		four directions.
func _disconnect_block(pos: Vector2i, dir_mask: int = 15) -> void:
	if not get_cell_source_id(0, pos) in [TILE_PIECE, TILE_BOX]:
		# only boxes/pieces have connections between blocks
		return
	var autotile_coord := get_cell_atlas_coords(0, pos)
	autotile_coord.x = PuzzleConnect.unset_dirs(autotile_coord.x, dir_mask)
	set_block(pos, get_cell_source_id(0, pos), autotile_coord)
...
static func random_veg_autotile_coord() -> Vector2i:
	return Vector2i(randi() % 18, randi() % 4)
