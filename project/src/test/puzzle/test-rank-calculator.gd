...
func before_each() -> void:
	Level.settings = LevelSettings.new()
	Level.settings.set_start_speed("0")
...
func test_max_lpm_fast_marathon() -> void:
	Level.settings.set_start_speed("F0")
	assert_almost_eq(_rank_calculator._max_lpm(), 68.90, 0.1)


func test_max_lpm_mixed_marathon() -> void:
	Level.settings.set_start_speed("0")
	Level.settings.add_speed_up(Milestone.LINES, 30, "A0")
	Level.settings.add_speed_up(Milestone.LINES, 60, "F0")
	Level.settings.set_finish_condition(Milestone.LINES, 100)
	assert_almost_eq(_rank_calculator._max_lpm(), 46.23, 0.1)


func test_max_lpm_mixed_sprint() -> void:
	Level.settings.set_start_speed("0")
	Level.settings.add_speed_up(Milestone.TIME_OVER, 30, "A0")
	Level.settings.add_speed_up(Milestone.TIME_OVER, 60, "F0")
	Level.settings.set_finish_condition(Milestone.TIME_OVER, 90)
	assert_almost_eq(_rank_calculator._max_lpm(), 50.98, 0.1)


func test_calculate_rank_marathon_300_master() -> void:
	Level.settings.set_finish_condition(Milestone.LINES, 300)
...
func test_calculate_rank_marathon_lenient() -> void:
	Level.settings.set_finish_condition(Milestone.LINES, 300, 200)
	PuzzleScore.level_performance.seconds = 240
	PuzzleScore.level_performance.lines = 60
	PuzzleScore.level_performance.box_score = 600
	PuzzleScore.level_performance.combo_score = 500
...
func test_calculate_rank_ultra_200() -> void:
	Level.settings.set_finish_condition(Milestone.SCORE, 200)
...
func test_calculate_rank_ultra_1() -> void:
	Level.settings.set_finish_condition(Milestone.SCORE, 1)
...
func test_calculate_rank_five_creatures_good() -> void:
	Level.settings.set_finish_condition(Milestone.CUSTOMERS, 5)
	Level.settings.set_start_speed("4")
...
func test_combo_score_per_line_death() -> void:
	Level.settings.set_finish_condition(Milestone.LINES, 200, 150)
...
func test_success_bonus_score() -> void:
	...
	PuzzleScore.level_performance.seconds = 240
	PuzzleScore.level_performance.score = 1160
	Level.settings.rank.success_bonus = 8
	
	# the player doesn't achieve the success condition; they get a worse grade
	Level.settings.set_success_condition(Milestone.SCORE, 2000)
	...
	Level.settings.set_success_condition(Milestone.SCORE, 1000)
	var rank2 := _rank_calculator.calculate_rank()
...
func test_success_bonus_seconds() -> void:
	...
	# the player doesn't achieve the success condition; they get a worse grade
	Level.settings.set_success_condition(Milestone.TIME_UNDER, 180)
	var rank1 := _rank_calculator.calculate_rank()
	assert_eq(RankCalculator.grade(rank1.seconds_rank), "S")

	# the player achieves the success condition; they get a better grade
	Level.settings.set_success_condition(Milestone.TIME_UNDER, 300)
	var rank2 := _rank_calculator.calculate_rank()
	assert_eq(RankCalculator.grade(rank2.seconds_rank), "S+")


func test_customer_combo() -> void:
	Level.settings.rank.customer_combo = 8
	Level.settings.rank.leftover_lines = 3
	Level.settings.set_finish_condition(Milestone.CUSTOMERS, 1)
