...
var spira: Spira3D setget set_spira
...
func _input(event: InputEvent) -> void:
	if not chatters and event.is_action_pressed("interact") and ChattableManager3D.get_focused():
...
func start_chat() -> void:
	...
	chatters = [ChattableManager3D.get_focused()]
	_update_visible()
	ChattableManager3D.set_focus_enabled(false)
...
func set_spira(new_spira: Spira3D) -> void:
	spira = new_spira
...
func _on_ChatUi_pop_out_completed() -> void:
	...
	chatters = []
	ChattableManager3D.set_focus_enabled(true)
	_update_visible()
