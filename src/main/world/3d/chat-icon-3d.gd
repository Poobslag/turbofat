...
func _ready() -> void:
	...
	ChattableManager3D.connect("focus_changed", self, "_on_ChattableManager_focus_changed")


func _on_ChattableManager_focus_changed() -> void:
	if not ChattableManager3D.is_focus_enabled():
		$ChatIconMesh.vanish()
	elif ChattableManager3D.is_focused(get_parent()):
		$ChatIconMesh.focus()
	else:
