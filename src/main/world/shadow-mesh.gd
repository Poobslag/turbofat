...
"""
A mesh which casts an appropriate shadow based on the customer's position and status.
"""

const DEFAULT_SHADOW_SCALE := Vector3(0.65, 0.85, 0.5)

# a scalar which makes the customer's shadow bigger/smaller
export (float) var _shadow_scale := 1.0 setget set_shadow_scale

"""
Sets a scalar which makes the customer's shadow bigger/smaller.
"""
