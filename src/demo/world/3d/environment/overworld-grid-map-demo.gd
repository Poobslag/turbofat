	...
	for x in range(-32, 32):
		for z in range(-32, 32):
			if x in range(-24, 24) and z in range(-24, 24) and randf() < 0.9:
				$GridMap.set_cell_item(x, 0, z, OverworldGridMap3d.TILE_BLOCK_ROCK)
				if randf() < 0.4:
					$GridMap.set_cell_item(x, 1, z, OverworldGridMap3d.TILE_PATH_CIRCLE)
				elif x in range(-16, 16) and z in range(-16, 16) and randf() < 0.1:
					$GridMap.set_cell_item(x, 1, z, OverworldGridMap3d.TILE_BLOCKHALF_ROCK)
...
func regenerate_path_world(regenerate: bool = true) -> void:
	...
	for z in range(0, path_layout.size()):
		for x in range(0, path_layout[z].length()):
			if path_layout[z][x] == ":":
				$GridMap.set_cell_item(x + path_offset.x, 1, z + path_offset.y, OverworldGridMap3d.TILE_PATH_CIRCLE)
	...
	for x in range(5, 7):
		for z in range(-7, -5):
			$GridMap.set_cell_item(x, 1, z, \
					OverworldGridMap3d.TILE_GRASS_TUFTS[randi() % OverworldGridMap3d.TILE_GRASS_TUFTS.size()])
